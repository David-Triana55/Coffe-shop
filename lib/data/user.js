...
export async function getAuctionsWonByUserId (userId) {
  try {
    ...
    `
    return auctions.rows[0].won_auctions
  } catch (e) {
    console.log(e)
  }
}

export async function getCountAndTotalSpentByUserId (userId) {
  try {
...
export async function getTotalBoughts (userId) {
  try {
    const result = await sql`
      SELECT 
        '5ef9a345-702c-4cab-848b-678201cbcf23' AS user_id,
        COUNT(*) AS totalOrders,
        SUM(totalSpent) AS totalSpent
      FROM (
        SELECT 
          b.id AS compra_id,
          (bd.quantity * bd.unit_price) AS totalSpent
        FROM bills b
        JOIN bills_details bd ON b.id = bd.bill_id
        JOIN products p ON bd.product_id = p.id
        WHERE b.user_id =   ${userId}

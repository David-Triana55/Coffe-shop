  ...
  try {
    const auction = await sql`
      SELECT 
        ...
        a.initial_price,
        a.minimum_increment,
        u_winner.id AS winner_id,
        CONCAT(u_winner.name, ' ', u_winner.last_name) AS winner_name,
      ...
      JOIN brands bnd ON p.brand_id = bnd.id
      JOIN users u_winner ON ap.user_buyer_id = u_winner.id
      LEFT JOIN bids b ON b.auction_id = a.id

      WHERE ap.auction_id = ${auctionId}
      GROUP BY 
        ap.id, ap.date, ap.final_price, ap.status,
        u_winner.id, u_winner.name, u_winner.last_name, u_winner.email,
        p.id, p.name, bnd.name, a.start_date, a.end_date, a.minimum_increment , a.initial_price , a.id

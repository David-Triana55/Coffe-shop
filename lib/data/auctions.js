...
export async function registerWinnersJob () {
  try {
    // Seleccionamos la puja más alta por subasta finalizada que aún no tiene compra
    const { rows: highestBids } = await sql`
      ...
      LEFT JOIN auctions_purchases ap ON ap.auction_id = a.id
      WHERE a.status = 'finished' AND ap.id IS NULL
      ORDER BY b.auction_id, b.amount DESC, b.created_at ASC
    `

    if (highestBids.length === 0) {
      console.log('No hay subastas finalizadas con pujas para registrar.')
      return []
    }
    ...
    for (const bid of highestBids) {
      const { auction_id, user_id, final_price } = bid

      // Insertar en auctions_purchases
      const { rows: purchase } = await sql`
        INSERT INTO auctions_purchases (auction_id, user_id, final_price, date)
        VALUES (${auction_id}, ${user_id}, ${final_price}, now())
        RETURNING *
      `

      // Actualizar la subasta a estado "pending"
      await sql`
        UPDATE auctions
        SET status = 'pending'
        WHERE id = ${auction_id}

...
export async function dailyAndMonthlySalesByBrand (brandId) {
  try {
    ...
    SELECT 
      bnd.id AS brand_id,
      bnd.name AS brand_name,
      SUM(CASE WHEN DATE(b.date) = CURRENT_DATE THEN bd.quantity * bd.unit_price ELSE 0 END) AS daily_sales,
      SUM(CASE WHEN DATE_TRUNC('month', b.date) = DATE_TRUNC('month', CURRENT_DATE) THEN bd.quantity * bd.unit_price ELSE 0 END) AS monthly_sales
    FROM bills b
    JOIN bills_details bd ON b.id = bd.bill_id
    JOIN products p ON bd.product_id = p.id
    JOIN brands bnd ON p.brand_id = bnd.id

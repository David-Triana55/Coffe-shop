...
export async function insertBill (id, date, mpPaymentId) {
  try {
    const bill = await sql`INSERT INTO bills (user_id, date, mp_payment_id) VALUES (${id},${date}, ${mpPaymentId}) RETURNING *`
    return bill.rows
  } catch (error) {
    console.log(error)
  }

...
export async function insertBill ({ userId, date, mpPaymentId }) {
  try {
    const bill = await sql`INSERT INTO bills (user_id, date, mp_payment_id) VALUES (${userId},${date}, ${mpPaymentId}) RETURNING *`
    return bill.rows
  } catch (error) {
    console.log(error)
  }
...
export async function insertDetailBill (billId, productId, quantity, price) {
  try {

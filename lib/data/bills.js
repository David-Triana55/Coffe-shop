...
export async function dailyAndMonthlySalesByBrand (brandId) {
  try {
    const { rows } = await sql`
      ...
      bnd.id AS brand_id,
      bnd.name AS brand_name,
      SUM(CASE WHEN DATE(s.sale_date) = CURRENT_DATE THEN s.total ELSE 0 END) AS daily_sales,
      SUM(CASE WHEN DATE_TRUNC('month', s.sale_date) = DATE_TRUNC('month', CURRENT_DATE) THEN s.total ELSE 0 END) AS monthly_sales
    FROM (
      SELECT 
        p.brand_id, 
        b.date AS sale_date, 

...
// export async function registerUser (name, lastName, number, password, email, type) {
//   try {
//     const user = await sql`INSERT INTO clientes (nombre_cliente, apellido, telefono, password, email, tipo_cliente) VALUES (${name}, ${lastName}, ${number}, ${password}, ${email}, ${type}) RETURNING *`
//     return user.rows
//   } catch (error) {
//     console.log(error)
//   }
...
// // ! ok
// export async function getCategoriesPrincipal () {
//   try {
//     const accesories = await sql`SELECT * FROM categorias  ORDER BY id_categoria ASC`
...
//   try {
//     const clientInfo = await sql`SELECT  name, last_name, email, phone FROM users WHERE id = ${id}`
...
// // // detallar un producto en la factura de la base de datos
...
// // // obtener la factura de la base de datos por id
...
// // // obtener los productos de la factura de la base de datos
...
//     INNER JOIN productos ON detalle_factura.id_producto = productos.id_producto WHERE id_factura = ${idFactura}`
//     return res.rows
//   } catch (error) {
//     console.log(error)
//   }
...
// // obtener el historial de la factura de la base de datos
...
//   try {
//     const res = await sql`
//     SELECT bills.id_factura ,facturas.fecha,
//     SUM(detalle_factura.cantidad * detalle_factura.precio_unitario) as total
//     FROM bills b
//     INNER JOIN detalle_factura ON facturas.id_factura = detalle_factura.id_factura
//     WHERE id_cliente = ${idFactura} GROUP BY facturas.id_factura;`
//     return res.rows
//   } catch (error) {
//     console.log(error)
//   }
...
// // transaccion para eliminar cliente y sus datos asociados

// export async function deleteClientDataTransaction (email) {
//   try {
//     await sql`BEGIN`

//     const client = await sql`SELECT id_cliente FROM clientes WHERE email = ${email}`
...
//     const clientId = await client.rows[0].id_cliente
...
//       WHERE id_factura IN (
//         SELECT id_factura FROM facturas WHERE id_cliente = ${clientId}
//       )
//     `

//     await sql`DELETE FROM facturas WHERE id_cliente = ${clientId}`

//     await sql`DELETE FROM clientes WHERE id_cliente = ${clientId}`
...
// // registrar una marca para un vendedor

// export async function registerBrandToSeller (name, brand, userId) {
//   try {
//     const res = await sql`INSERT INTO marcas (nombre_marca, imagen, id_vendedor) VALUES (${name}, ${brand}, ${userId}) RETURNING *`
//     return res.rows
//   } catch (error) {
//     console.log(error)
//   }

...
export default async function PageAccesories ({ params }) {
  const accessory = await params.items.split('-').join(' ')
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/accessories/${accessory}`, { cache: 'no-store' })
  const data = await res.json()
  const { products, info } = data
  const typesCoffee = formatCategory(accessory)
          ...
          <div className='grid md:grid-cols-2 gap-12 items-center max-w-6xl mx-auto'>
                ...
                {info?.image_url
                  ...
                  <div className='relative w-full h-full'>
                    <Image
                      ...
                      alt={`Logo de ${typesCoffee || accessory}`}
                      fill
                      priority
            ...
            <div className='space-y-6 text-center md:text-left'>
              <h1 className='text-5xl md:text-6xl font-bold bg-gradient-to-r from-[#3E2723] to-[#5D4037] bg-clip-text text-transparent'>
                {typesCoffee || accessory}
              </h1>
              <p className='text-lg text-[#5D4037] leading-relaxed'>
                {info?.description || 'Descubre todos los productos de este accesorio seleccionado especialmente para ofrecerte la mejor experiencia cafetera.'}
              </p>
        ...
        <div className='container mx-auto px-4'>
          <div className='text-center mb-16'>
            <h2 className='text-4xl font-bold text-[#3E2723] mb-4'>
              Productos de <span className='text-[#33691E]'>{typesCoffee || accessory}</span>
            </h2>
